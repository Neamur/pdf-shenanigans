...
class PDF(FPDF):
    ...
    def footer(self):
        ...
        # self.text_mode.FILL 
        # self.text_mode.FILL_STROKE
        # Printing page number:
        # self.cell(100,0, align="L",fill=False)
        # self.cell(w=0,h=0,text=f"{Page_no}/{total_pages}", align="R", fill=True)
        # self.write_html(f'<font color="#fca311"> <p align="right">{Page_no}/{total_pages}</p> </font>')
        if choice == "Fill_Stroke":
            ...
            with self.local_context():  # to fill with bg.
                ...
                self.set_fill_color(252, 163, 17) # cell background color
                # self.cell(w=0,h=0,new_x="END")      # change new_x based on the alignment.
                setting_y_pos(self)
                margin_pos(self)
                # self.set_x(-20)                         # need to use this line only when doing right margin.
                self.cell(text=f"{Page_no}/{total_pages}",fill=True)
            # self.write_html(f'<font_size = "14"> <font face="helvetica"> <font color="#fca311"> <p align="right">{Page_no}/{total_pages}</p>')

def new_content(pg_dimension):      #pg_dimension takes in the page dimension as a tuple for each page in the pdf to be numbered.
    pdf = PDF()
    pdf.add_page(format=pg_dimension)      #format=pg_dimension
    # print(pdf.w)
    # print(pdf.h)
...
for selected_pdf in selected_pdfs:
    ...
    while ON_PAGE_INDEX < len(reader.pages):
        ...
        print(page_dimension.lower_right)
        print(page_dimension.lower_left)
        print(page_dimension.upper_right)
        print(page_dimension.upper_left)
        page_width_height = (page_dimension[2]//WIDTH_RATIO, page_dimension[3]//HEIGHT_RATIO)
        print("width : ",page_dimension[2],"height: ",page_dimension[3])
        ...
        #     else:
        #         print('Landscape')
        page_overlay = PdfReader(new_content(page_width_height)).pages[0]        # page_width_height as the arg for new_content()
        rotated_page_overlay = page_overlay.rotate(90)
